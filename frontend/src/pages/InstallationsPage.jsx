...
export default function InstallationsPage() {
  ...
  const loadAll = async () => {
    setLoading(true)
    try {
      const response = await serviceOrderApi.getServiceOrders({ service_type: 'instalacion' })
      setOrders(response.data.results || response.data)
    } catch {
      toast.error('No se pudieron cargar las órdenes de instalación')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-600">
          Detalle de las órdenes de servicio clasificadas como instalación.
        </p>
      </div>
      ...
      <div className="rounded-lg border border-gray-200 bg-white overflow-hidden shadow-sm">
        <div className="border-b border-gray-200 px-4 py-3 flex items-center justify-between">
          <div>
            <h3 className="text-sm font-semibold text-gray-900">Órdenes de instalación</h3>
            <p className="text-xs text-gray-500">No se registran instalaciones aquí; se consultan las órdenes de servicio.</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="px-4 py-6 text-sm text-gray-500">No hay órdenes de instalación registradas.</div>
        ) : (
